...
class ModelArguments:
    ...
    cross_attention_use_policy: bool = field(
        default=True,
        metadata={"help": "Enable learned latent policy bias in decoder cross-attention."},
    )
    cross_attention_policy_hidden_dim: Optional[int] = field(
        default=None,
        metadata={"help": "Hidden size of the cross-attention policy MLP. Defaults to head_dim when unset."},
    )
    cross_attention_policy_init_scale: float = field(
        default=0.1,
        metadata={"help": "Initialization scale for cross-attention policy parameters and gate."},
    )
    cross_attention_policy_clip_beta: float = field(
        default=1.0,
        metadata={"help": "Clipping range beta used to stabilize learned policy bias."},
    )
...
def run_bart():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        json_config_path = os.path.abspath(sys.argv[1])
        model_args, data_args, training_args = parser.parse_json_file(json_file=json_config_path)
    else:
    ...
    def _resolve_local_path(path_value: Optional[str]) -> Optional[str]:
        ...
        if json_config_path is not None:
            json_dir = os.path.dirname(json_config_path)
            candidate_paths.append(os.path.abspath(os.path.join(json_dir, path_value)))
            candidate_paths.append(os.path.abspath(os.path.join(json_dir, basename)))
            candidate_paths.append(os.path.abspath(os.path.join(json_dir, "dataset", basename)))

        candidate_paths.append(os.path.abspath(os.path.join(script_dir, path_value)))
        candidate_paths.append(os.path.abspath(os.path.join(script_dir, basename)))
        candidate_paths.append(os.path.abspath(os.path.join(script_dir, "dataset", basename)))
        candidate_paths.append(os.path.abspath(os.path.join(cwd_dir, path_value)))
        candidate_paths.append(os.path.abspath(os.path.join(cwd_dir, basename)))
        candidate_paths.append(os.path.abspath(os.path.join(cwd_dir, "dataset", basename)))

        for candidate in candidate_paths:
            if os.path.exists(candidate):
                return candidate

        # Last resort: lightweight recursive lookup near expected project roots.
        search_roots = []
        if json_config_path is not None:
            search_roots.append(os.path.dirname(json_config_path))
        search_roots.extend([script_dir, cwd_dir])
        for root in search_roots:
            recursive_matches = glob.glob(os.path.join(root, "**", basename), recursive=True)
            if recursive_matches:
                return os.path.abspath(recursive_matches[0])

        return path_value

    # Resolve local file paths provided in JSON relative to the JSON location.
    data_args.dataset_name = _resolve_local_path(data_args.dataset_name)
    data_args.train_file = _resolve_local_path(data_args.train_file)
    data_args.validation_file = _resolve_local_path(data_args.validation_file)
    data_args.test_file = _resolve_local_path(data_args.test_file)
    if data_args.dataset_name is not None:
        looks_like_path = (
            "/" in data_args.dataset_name
            or "\\" in data_args.dataset_name
            or data_args.dataset_name.endswith((".json", ".csv", ".jsonl"))
        )
        if looks_like_path and not os.path.exists(data_args.dataset_name):
            checked_paths = []
            if json_config_path is not None:
                checked_paths.append(os.path.abspath(os.path.join(os.path.dirname(json_config_path), data_args.dataset_name)))
            checked_paths.append(os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), data_args.dataset_name)))
            checked_paths.append(os.path.abspath(os.path.join(os.getcwd(), data_args.dataset_name)))
            raise FileNotFoundError(
                f"Dataset file not found: {data_args.dataset_name}. "
                "Checked common locations: "
                + ", ".join(checked_paths)
                + ". If this is a local file, provide an absolute path or a path relative to the JSON config file."
            )
    for split_file_name, split_file_path in [
        ("train_file", data_args.train_file),
        ("validation_file", data_args.validation_file),
        ("test_file", data_args.test_file),
    ]:
        if split_file_path is not None and not os.path.exists(split_file_path):
            raise FileNotFoundError(f"{split_file_name} not found: {split_file_path}")
    ...
    config.cross_attention_use_policy = model_args.cross_attention_use_policy
    config.cross_attention_policy_hidden_dim = model_args.cross_attention_policy_hidden_dim
    config.cross_attention_policy_init_scale = model_args.cross_attention_policy_init_scale
    config.cross_attention_policy_clip_beta = model_args.cross_attention_policy_clip_beta
    ...
    def _load_json_dataset(split_name: str, file_path: Optional[str] = None, subset_split: Optional[str] = None):
        if file_path is not None:
            return load_dataset('json', data_files=file_path, split=subset_split if subset_split is not None else 'train')

        if data_args.dataset_name is None:
            raise ValueError(
                f"No dataset source provided for split '{split_name}'. Set dataset_name or the corresponding file path."
            )
        ...
        try:
            ...
            raise ValueError(
                f"Failed to load split '{split_name}' from {data_args.dataset_name}. "
                "If this file is a flat JSON/JSONL dataset rather than an object with top-level split keys like "
                "'train'/'eval'/'test', set train_file/validation_file/test_file in the config instead of dataset_name."
            ) from exc
    ...
    if training_args.do_train:
        ...
        train_dataset = _load_json_dataset('train', file_path=data_args.train_file)
    ...
    if training_args.do_eval:
        ...
        eval_dataset = None
        dataset_name_str = data_args.dataset_name or ""
        if data_args.validation_file is not None:
            eval_dataset = _load_json_dataset('eval', file_path=data_args.validation_file)
        elif 'RADIOLOGY' in dataset_name_str:
            # eval set too large, therefore cutting off. 
            # test set remains same size.
            eval_dataset = _load_json_dataset('eval', subset_split='train[:30%]')
        else:
            eval_dataset = _load_json_dataset('eval')
    ...
    if training_args.do_predict:
        ...
        predict_dataset = _load_json_dataset('test', file_path=data_args.test_file)
